"""
Test start_graph_and_communicate_python.
"""
...
def test_start_graph_and_communicate_python():
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "start_graph_and_communicate_python_app/ten_packages/system/"
    ...
    if not is_started:
        print(
            "The start_graph_and_communicate_python is not started after 30 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print(
            "The exit code of start_graph_and_communicate_python: ",
            exit_code,
        )
