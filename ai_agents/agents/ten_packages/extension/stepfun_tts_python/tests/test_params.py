...
def test_params_passthrough(MockStepFunTTSWebsocket):
    """
    Tests that custom parameters passed in the configuration are correctly
    forwarded to the StepFunTTSWebsocket client constructor.
    """
    ...
    tester.set_test_mode_single("stepfun_tts_python", json.dumps(real_config))
    ...
    # --- Assertions ---
    # Check that the StepFunTTSWebsocket client was instantiated exactly once.
    MockStepFunTTSWebsocket.assert_called_once()
    ...
    call_args, call_kwargs = MockStepFunTTSWebsocket.call_args
