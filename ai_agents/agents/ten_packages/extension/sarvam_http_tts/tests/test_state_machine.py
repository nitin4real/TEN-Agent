...
def test_sequential_requests_state_machine(MockSarvamTTSClient):
    """
    ...
    MockSarvamTTSClient.return_value = mock_instance
    ...
    config = {
        "params": {
            "api_subscription_key": "test_api_key_for_state_machine",
    ...
    tester.set_test_mode_single("sarvam_http_tts", json.dumps(config))
    tester.run()
...
@patch("sarvam_http_tts.extension.SarvamTTSClient")
def test_request_state_transitions(MockSarvamTTSClient):
    """
    ...
    config = {
        "params": {
            "api_subscription_key": "test_api_key_for_state_transitions",
            "target_language_code": "en-IN",
            "speaker": "anushka",
            "pitch": 0.0,
            "pace": 1.0,
