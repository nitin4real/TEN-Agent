...
def test_empty_params_fatal_error():
    ...
    empty_params_config = {
        "params": {"api_subscription_key": "", "target_language_code": "hi-IN"}
    }

    tester = ExtensionTesterEmptyParams()
    tester.set_test_mode_single(
        "sarvam_http_tts", json.dumps(empty_params_config)
    )
...
def test_invalid_api_key_error(MockAsyncClient):
    ...
    mock_client.post.side_effect = Exception(
        "Client error '401 Unauthorized' for url 'https://api.sarvam.ai/text-to-speech'"
    )
    ...
    invalid_key_config = {
        "params": {
            "api_subscription_key": "invalid_api_key_test",
    ...
    assert (
        vendor_info.get("vendor") == "sarvam"
    ), f"Expected vendor 'sarvam', got {vendor_info.get('vendor')}"
