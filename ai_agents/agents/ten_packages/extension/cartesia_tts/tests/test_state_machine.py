...
def test_sequential_requests_state_machine(MockCartesiaTTSClient):
    ...
    # cartesia_tts uses: async for audio_chunk, event in self.client.get(text)
    # Cartesia first yields TTFB metric, then audio chunks, then END event
    async def mock_get_request1(_text: str):
        """Mock get() for first request - yields TTFB, audio chunks then END event"""
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding TTFB metric for request 1")
        yield 50, EVENT_TTS_TTFB_METRIC  # 50ms TTFB
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding chunk 1 for request 1")
        yield b"mock_audio_chunk_1", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding chunk 2 for request 1")
        yield b"mock_audio_chunk_2", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding chunk 3 for request 1")
        yield b"mock_audio_chunk_3", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding END event for request 1")
        yield None, EVENT_TTS_END

    async def mock_get_request2(_text: str):
        """Mock get() for second request"""
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding TTFB metric for request 2")
        yield 45, EVENT_TTS_TTFB_METRIC  # 45ms TTFB
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding chunk 1 for request 2")
        yield b"mock_audio_chunk_1", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding chunk 2 for request 2")
        yield b"mock_audio_chunk_2", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding END event for request 2")
        yield None, EVENT_TTS_END

    # Create a stateful mock that alternates between requests
    call_count = [0]

    def mock_get(text: str):
        call_count[0] += 1
        if call_count[0] == 1:
            print(
                f"  → Mock: get() called for first request (text: {text[:20]}...)"
            )
            return mock_get_request1(text)
        else:
            print(
                f"  → Mock: get() called for second request (text: {text[:20]}...)"
            )
            return mock_get_request2(text)

    mock_instance.get = mock_get
    mock_instance.cancel = MagicMock()
    mock_instance.reset_ttfb = MagicMock()

    # Mock async methods (required for Cartesia)
    ...
    # Create test configuration for Cartesia TTS
    config = {
        "params": {
    ...
    print("  → Using test config with mock client")
...
def test_request_state_transitions(MockCartesiaTTSClient):
    ...
    # Simple mock for single request - Cartesia sends TTFB first
    async def mock_get(_text: str):
        """Mock a simple TTS request"""
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding TTFB metric")
        yield 50, EVENT_TTS_TTFB_METRIC
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding audio chunk")
        yield b"mock_audio_chunk", EVENT_TTS_RESPONSE
        await asyncio.sleep(0.01)
        print("  → Mock: Yielding END event")
        yield None, EVENT_TTS_END
