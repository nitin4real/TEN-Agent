...
# Event constants (matching google_tts.py)
...
def test_sequential_requests_state_machine(MockGoogleTTS):
    """
    ...
    mock_client_instance.get = streamer.get
    mock_client_instance.get_streaming = streamer.get
    mock_client_instance.clean = MagicMock()
...
@patch("google_tts_python.extension.GoogleTTS")
def test_request_state_transitions(MockGoogleTTS):
    """
        ...
        class Session:
            ...
            async def get_audio_data(self):
                ...
                if self.call_count == 1:
                    # Return one chunk
                    return b"audio_chunk", EVENT_TTS_RESPONSE, 123
                else:
                    # Signal completion
                    return None, EVENT_TTS_REQUEST_END, None
