...
from .anam import AgoraAnamRecorder
...
@dataclass
class AnamAvatarConfig(BaseConfig):
    agora_appid: str = ""
    agora_appcert: str = ""
    channel: str = ""
    ...
    def validate_params(self) -> None:
        ...
        required_fields = {
            ...
            "anam_base_url": self.anam_base_url,
...
class AnamAvatarExtension(AsyncExtension):
    def __init__(self, name: str):
        ...
        self.recorder: AgoraAnamRecorder = None
    ...
    async def on_start(self, ten_env: AsyncTenEnv) -> None:
        ...
        try:
            self.config = await AnamAvatarConfig.create_async(ten_env)
            ...
            ten_env.log_info(f"Avatar ID: {self.config.anam_avatar_id}")
            ten_env.log_info(
            ...
            if self.config.anam_pod:
                ten_env.log_info(f"Pod: {self.config.anam_pod}")

            recorder = AgoraAnamRecorder(
                api_key=self.config.anam_api_key,
                base_url=self.config.anam_base_url,
                ...
                cluster=self.config.anam_cluster,
                pod=self.config.anam_pod,
    ...
    async def on_data(self, ten_env: AsyncTenEnv, data: Data) -> None:
        ...
        if data_name == "tts_audio_end":
            ...
            if json_str:
                ...
                ten_env.log_info(
                    f"[ANAM_TTS_END] Received tts_audio_end: "
